...
app.get("/api", (c) =>
  c.json({
    ...
    endpoints: {
      ...
      crypto_hosted: "POST /crypto-checkout { plan, email?, apiKey? } — hosted crypto invoice page (NOWPayments; active when configured)",
...
app.route("/", nowpaymentsRouter);  // POST /crypto-checkout + /nowpayments/ipn (hosted crypto for humans)
  ...
  scheduled: async (_event: unknown, env: Env, ctx: { waitUntil: (p: Promise<unknown>) => void }) => {
    ctx.waitUntil(
      Promise.all([
        ...
        import("./services/nowpayments.js").then((m) => m.expireNowPaymentsCredits(env)),
      ]),
    );
  },
};
